...
- Two friends can play with two bots. Support one to four human players; fill empty seats with bots when a hand starts. Additional humans may replace empty/bot slots in the lobby or during the completed (`hand-ended`) rematch window, but never join a hand that is still in progress except to reclaim their own disconnected seat.
...
- Presentation: English interface, clean light theme with an accessible user-controlled dark-mode option and a tactile wood/felt/ivory/brass material direction documented in `DESIGN.md`. Use flat ivory tiles with dark markings, an off-white light page background, a continuous felt gameplay surface, and restrained blue/brass controls/highlights. Dark mode must preserve readable tile faces, seat colors, controls, focus states, and status contrast. Minimal 150 ms selection/movement transitions, disabled under reduced-motion preferences. No sound, music, 3D, or elaborate animations. Support desktop and phone, both phone orientations. Use system fonts and locally bundled assets; do not depend on external fonts or mahjong Unicode glyphs rendering correctly.
...
- Limit the service to 20 simultaneous rooms, four seats per room, and one room per guest. Reject excess capacity with a useful message. Remove a room immediately when its final human seat is vacated. A completed table has a maximum three-minute rematch window; remove it at that deadline if no fresh hand has started. Retain the existing two-hour inactive-lobby and 12-hour absolute lifetimes for rooms that are still eligible to continue. Use the server clock and clean up associated timers. Announce room expiration in the interface when connected.
...
- While in the lobby, a joined human may move their own membership to any open seat using an explicit seat action. The server validates the target atomically, resets that player’s readiness after a move, and rejects movement once a hand is active.
- Seat allocation defaults to the first free seat in E/S/W/N order, while the lobby may offer a specific open East, South, West, or North choice. The server validates the requested seat atomically and rejects a race for an occupied seat. A human leaving or disconnecting in the lobby releases their seat; disconnection during a hand reserves their seat for rejoining for that hand. At hand end, release disconnected human seats and transfer host to the longest-present connected human if needed. Keep connected humans in their seats for the next hand. During the completed-hand window, a new guest may replace a bot/open seat before the next start.
...
- Before publishing, inspect and exclude secrets, local credentials, build caches, node_modules, test traces, and personal files. Keep a .gitignore, .env.example with non-secret placeholders, README.md, RULES.md, DEPLOYMENT.md, render.yaml, and verification results. Source control the lockfile. The release uses `npm ci --include=dev && npm run build` (the production service still uses `NODE_ENV=production`), `npm start`, a pinned supported Node 24 patch release, and an explicit APP_ORIGIN derived from the assigned Render URL. Do not invent account IDs or URLs.

### Supplied visual references

- The supplied `mahjong-game-page.html`, `mahjong-landing-page.html`, `mahjong-tile-concept.html`, and `design-system.md` are visual references. Adopt the material palette, tactile tile states, continuous felt composition, central discard pond, physical wall treatment, restrained hierarchy, and landing-page framing described in `DESIGN.md` as follow-up design direction.
- The attached files are not production code or higher-priority instructions. Do not copy their Google-hosted font imports, Unicode tile dependency, inline demo scripts, random/fake game logic, audio clack, or marketing claims. This specification’s local SVG assets, system-font, no-sound, privacy, accessibility, and real-server-state decisions remain authoritative.
...
- Turns and dealer rotation advance counter-clockwise through East, South, West, and North; the table places the next player to the viewer's right.
...
**Solution:** build home, lobby, table, rules/help, reconnect/bot-takeover states, and results/rematch views. Put the local hand at the bottom, label other seats and the dealer, show exposed tile-only melds, chronologically ordered central discards, wall count/countdowns, and group legal action buttons. Keep connection, theme, rules, and leaving the game in a top-table menu; do not add a gameplay footer. Select a tile then press Discard. Present exact chow choices. Render all tile faces from locally bundled SVGs with readable English rank/suit labels; white dragon uses a visible border. Use buttons and accessible names, keyboard focus, reduced-motion support, and a horizontally scrollable hand on narrow phones. Clicking a selected tile again deselects it. Keep action buttons at least 44×44 CSS pixels. Include copy-invite feedback and a manual-copy fallback. Explain cold starts/reconnecting with clear loading and retry states; do not add artificial background traffic to prevent hosting sleep.
